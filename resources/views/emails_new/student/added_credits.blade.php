		...
		<tr>
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
				...
				<tr>
					...
					There are new credits in your account as follows:
					<br>

                <ul>
                    <li>{{ $creditsModel->credits }} have been deposited for amount Rs.{{ inrFormat($creditsModel->amount_paid) }}/-</li>

                    @if($creditsModel->anyCoupon)
                    <li>
                        And we have deposited {{ $creditsModel->anyCoupon->credits }} free credits since you are eligible for a coupon: {{ $creditsModel->anyCoupon->coupon->description }}
                    </li>
                    @endif
                </ul>
