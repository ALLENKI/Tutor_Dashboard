      ...
      <div class="m-portlet m-portlet--mobile">
        ...
        <div class="m-portlet__body">
          ...
          <el-form :model="addTopicForm" :rules="addTopicRules" ref="addTopicForm" label-width="120px" class="demo-addTopicForm">
            ...
            <el-form-item label="Topic Aprrove">

              <el-checkbox v-model="checked"> Approve </el-checkbox>
            ...
            <el-form-item label="Description" prop="description">
              <vue-editor id="te_editor" class="body_editor" v-model="addTopicForm.description" :editorToolbar="customToolbar"></vue-editor>
            </el-form-item>

            <el-form-item>
              <el-button type="primary" @click="submitForm('addTopicForm')">Update</el-button>
  ...
  methods: {
    getTopic() {
      axios
        ...
        .then(response => {
          ...
          this.addTopicForm.name = this.topic.name;
          this.checked = this.topic.approve;
          this.visibilityChecked = this.topic.visibility;
          this.addTopicForm.status = this.topic.status;
          this.addTopicForm.description = this.topic.description;
          this.addTopicForm.subcategory = [
            this.topic.category_id,
            this.topic.subject_id,
            this.topic.sub_category_id
          ];

          this.units = _.map(this.topic.units, function(unit) {
            unit.timestamp = (Date.now() / 1000) | 0;
            unit.timestamp = unit.timestamp + unit.id;
            return unit;
          });
        })
        .catch(error => {
    ...
    addTopic() {
      ...
      let payload = {
        ...
        units: this.units,
        approve: this.checked,
        visibility: this.visibilityChecked,
      };

      axios
        .put("course_catalog/topics/" + this.topic.id, payload)
        .then(response => {
